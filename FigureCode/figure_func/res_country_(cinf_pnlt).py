# -*- coding: utf-8 -*-
"""
Country infection-control penalty across the estimated R0 range.
"""
...
def draw(anal_data, **kwargs):
    ...
    for country in COUNTRIES:
        ...
        pnlt_stats = _mean_ci(
            anal_data['necs_from_country'][sheet_name][f'{country}_none_pnlt'] * 100
        )
    ...
    prefix = (
        f"necs_from_param_(delay)-{country_abbr}_c_{acc_type}"
        f"{'_dur' if dur else ''}"
    )
    ...
    ax.plot(x_theory, theory, color=green, linestyle='--',
            linewidth=1.0, alpha=0.35, zorder=1,
            label=r'Theoretical $\Phi_{\mathrm{c}}^{(\mathrm{d})}$')
    ax.errorbar(
        r0_mean,
        pnlt_mean,
    ...
    ax.plot(
        ...
        markersize=3.8,
        linewidth=1.55,
        label=r'Country-specific $\Phi_{\mathrm{c}}^{(\mathrm{d})}$',
        zorder=3,
    )
    ...
    label_offsets = {
        'Ireland': (0.02, -4.2),
        'Japan': (-0.10, 2.0),
        'United Kingdom': (-0.07, 4.2),
        'France': (-0.02, 7.2),
        'Germany': (0.01, 5.5),
        'United States': (0.02, 4.0),
        'Spain': (0.02, 4.3),
        'Austria': (0.02, 4.2),
        'Israel': (-0.16, 0),
    ...
    ax.legend(frameon=False, fontsize=7, loc='upper right',
              bbox_to_anchor=(0.99, 0.99), handlelength=2.2)
