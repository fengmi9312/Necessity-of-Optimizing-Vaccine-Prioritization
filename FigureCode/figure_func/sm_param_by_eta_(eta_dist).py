...
def draw(anal_data):
    ...
    ax = axes[0][0]
    plt.sca(ax)
    plt.bar(np.arange(16), np.array([0.88, 0.95, 0.96, 0.97, 0.97, 0.96, 0.95, 0.95, 0.94, 0.93, 0.91, 0.88, 0.75, 0.70, 0.65, 0.60]) * 100, color = 'tab:orange', width = 0.8, edgecolor='tab:gray', linewidth = 1)
    ax.set_ylim(0, 100)
    ax.spines['left'].set_bounds(0, 100)
    ax.set_yticks(np.arange(0, 101, 50))
    figure_setting.set_xylabel(ax, 'Age group', 'Vaccine efficacy (%)', fontsize = label_fontsize, xlabel_coords = -0.23, ylabel_coords = -0.12)
