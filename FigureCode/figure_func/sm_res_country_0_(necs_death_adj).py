# -*- coding: utf-8 -*-
"""
Country-specific IFR-normalized necessity for minimizing deaths against the fitted R0 landscape.
"""
...
def draw(anal_data, **kwargs):
    ...
    country_task = kwargs.pop('country_task', 'necs_from_country_35')
    ...
    for country in COUNTRIES:
        ...
        r0_stats = _mean_ci(anal_data[country_task]['r0'][country])
        r0_mean.append(r0_stats[0])
        r0_lower.append(r0_stats[1])
        r0_upper.append(r0_stats[2])

        necs_key = f'{country}_none_necs'
        necs_stats = _mean_ci(anal_data[country_task][sheet_name][necs_key] * 100)
    ...
    landscape_ifr = (
        country_data[country_for_landscape]['populations']
        @ country_data[country_for_landscape]['ifrs']
    )
    landscape = anal_data['necs_from_r0_factor'][landscape_key][param_idx] * 100 / landscape_ifr
    ax.plot(x, landscape, color='0.55', linewidth=1.15,
            label='Theoretical landscape', zorder=1)
    ...
    cluster_countries = {'Japan', 'United Kingdom', 'France', 'Germany', 'United States'}
    axins = ax.inset_axes([0.60, 0.08, 0.30, 0.27])
    axins.errorbar(
        r0_mean,
        necs_mean,
        xerr=[r0_mean - r0_lower, r0_upper - r0_mean],
    ...
    zoom_x0, zoom_x1 = 1.55, 2.25
    zoom_y0, zoom_y1 = 7, 15
    ...
    cluster_label_offsets = {
        'Japan': (-0.1, -0.75),
        'United Kingdom': (-0.08, -0.95),
        'France': (0.03, 0.65),
        'Germany': (0.03, -1),
        'United States':  (0.04, 0.8),
    }
    for country, x_val, y_val in zip(COUNTRIES, r0_mean, necs_mean):
    ...
    ax.xaxis.set_major_locator(LogLocator(base=10, subs=(1, 2, 4, 6)))
    ax.xaxis.set_major_formatter(FuncFormatter(_format_log_tick))
    ax.xaxis.set_minor_formatter(NullFormatter())
    ax.grid(True, color='0.88', linestyle=':', linewidth=0.55)
    ax.set_axisbelow(True)

    ax.set_title('Country-specific IFR-normalized necessity\n'r'for minimizing deaths on $R_0$ landscape',
