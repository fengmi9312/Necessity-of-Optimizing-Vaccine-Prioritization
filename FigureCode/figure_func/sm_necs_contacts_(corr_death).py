...
def draw(anal_data, **kwargs):
    ...
    for ax_idx, expr_param in enumerate(['1', '13']):
        ...
        for param_idx, param_val in enumerate(y):
            ...
            X.append(x * yyy / xxx)
            if param_idx < 40: corr_z.append((np.arccos(anal_data[corr_anal_name][corr_sheet_name]['optimal_indx']) - np.arccos(anal_data[corr_anal_name][corr_sheet_name]['optimal_dirx'])) / np.arccos(anal_data[corr_anal_name][corr_sheet_name]['effect_corrx']))
