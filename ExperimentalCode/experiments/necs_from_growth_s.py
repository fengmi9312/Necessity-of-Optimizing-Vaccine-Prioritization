# -*- coding: utf-8 -*-
"""
Created on Sat Mar 29 00:35:48 2025
...
def execute(expr_param, file_idx):
    ...
    delay = (file_idx // 4) * basic_params.day_div
        ...
        for r0_idx, r0 in enumerate(np.exp(np.arange(40) * 0.075)):
            for mean_idx, mean_val in enumerate(mean_list):
                ...
                if not cache_generator.check_cache(expr_param, file_idx, param_name):
                    ...
                    single_res.update({'transmission_params': {'params': pd.Series([r0, growth_rate, delay_day, mean_val], index=['r0', 'growth_rate', 't_resp', 't_gen'])}})
