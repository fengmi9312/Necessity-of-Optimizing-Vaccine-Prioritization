...
alloc_idx = []
for i in range(16):
    if res[0]['alloc'][i] > 0.1 and res[0]['alloc'][i] < 0.9 * simu.getc_s()[i]:
        alloc_idx.append(i)

u1 = (res[1]['alloc'] - res[0]['alloc']) / np.linalg.norm(res[1]['alloc'] - res[0]['alloc'])  # v1 到 v2 的单位向量
u2 = find_x(res[1]['alloc'] - res[0]['alloc'], calc_params['populations'], 
       calc_params['populations'] @ simu.calc_vac_prdt_grad(res[0]['alloc']), alloc_idx)
#u2 = (np.zeros(16) - res[0]['alloc']) - (np.zeros(16) - res[0]['alloc']) @ u1 * u1

u2 /= np.linalg.norm(u2)

contact_arr = np.sum(calc_params['contacts'], axis = 1)
...
x = []
while True:
    x_coef = alloc_step * idx
    alloc = res[0]['alloc'] + x_coef * u1 + y_coef * u2
    if np.any(alloc > simu.getc_s()) or np.any(alloc < 0): break
    alloc_coefs.append([x_coef, y_coef])
    x.append(pearsonr(alloc * calc_params['populations'], contact_arr)[0])
    prdts.append(simu.calc_vac_prdt_target(alloc, 'd'))
    coef_line_a.append([x_coef, y_coef])
    print(x_coef, y_coef)
    idx += 1

for coef_idx, coef in enumerate(coef_line_a):
    if coef_idx == 0: coef_line_b = []
    elif coef_idx == len(coef_line_a) - 1: coef_line_c = []
    else: pass
    x_coef = coef[0]
    for i in range(2):
        idx = -1 if i == 0 else 0
        while True:
            y_coef = alloc_step * idx
            alloc = res[0]['alloc'] + x_coef * u1 + y_coef * u2
            if np.any(alloc > simu.getc_s()) or np.any(alloc < 0): break
            alloc_coefs.append([x_coef, y_coef])
            x.append(pearsonr(alloc * calc_params['populations'], contact_arr)[0])
            prdts.append(simu.calc_vac_prdt_target(alloc, 'd'))
            if coef_idx == 0: coef_line_b.append([x_coef, y_coef])
            if coef_idx == len(coef_line_a) - 1: coef_line_c.append([x_coef, y_coef])
            else: pass
            print(x_coef, y_coef)
            if i == 0: idx -= 1
            else: idx += 1

for coef_line_idx, coef_line in enumerate([coef_line_b, coef_line_c]):
    if len(coef_line) <= 1: continue
    for coef_idx, coef in enumerate(coef_line):
        y_coef = coef[1]
        idx = -1 if coef_line_idx == 0 else 1
        while True:
            x_coef = coef[0] + alloc_step * idx
            alloc = res[0]['alloc'] + x_coef * u1 + y_coef * u2
            if np.any(alloc > simu.getc_s()) or np.any(alloc < 0): break
            alloc_coefs.append([x_coef, y_coef])
            x.append(pearsonr(alloc * calc_params['populations'], contact_arr)[0])
