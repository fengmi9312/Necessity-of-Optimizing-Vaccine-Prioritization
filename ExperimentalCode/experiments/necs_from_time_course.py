...
def execute(expr_param, file_idx):
    ...
    for country_idx, country in enumerate(countries):
        ...
        param_name = f'{expr_param}_{basic_params.country_abbr[country]}_14'
        print(f'Begin excuting {param_name}')
        alloc_map, dist_map = {}, {}
        for sttg in sttgs:
            print('Strategy:', sttg)
            if sttg != 'no_vac':
                alloc_map[f'{sttg}_dd'] = simu_once.optimize_time_course_vac_alloc(vac_avail = vac_avail, vac_dur = 14, vac_gap = 100, optm_dir = (sttg[:3] == 'min'), 
                                                                                   target = sttg[-1], disp = True, tol=1e-20)['alloc']
                dist_map[f'{sttg}_dd'] = simu_once.calc_time_course_vac_prdt(alloc_map[f'{sttg}_dd'], vac_dur = 14, vac_gap = 100)
            else:
