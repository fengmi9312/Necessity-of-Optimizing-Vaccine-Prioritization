...
def execute(expr_param, file_idx):
    ...
    for country_idx, country in enumerate(countries):
        ...
        if simu_once.getc_c_tot() < c_perct * (steady_c - init_c) + init_c:
            while True:
                ...
                if simu_once.getc_c_tot() >= c_perct_end * (steady_c - init_c) + init_c: 
                    ...
                    break
        delay_ratio_int = file_idx % 40
        calc_params['delay'] = int((end_time_int - init_time_int) * delay_ratio_int / 20)
        simu_once = sir_delta(**calc_params)
        if simu_once.getc_c_tot() < c_perct * (steady_c - init_c) + init_c:
            while True:
                simu_once.spread_once()
        ...
        param_name = f'{expr_param}_{basic_params.country_abbr[country]}_{delay_ratio_int}'
