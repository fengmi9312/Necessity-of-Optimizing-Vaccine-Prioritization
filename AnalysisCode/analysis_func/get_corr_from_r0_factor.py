...
def analyze(expr_data):  
    ...
    anal_data = {}
    task_names = ['necs_from_param_(delay)', 'necs_from_param_(c_perct)',  'necs_from_param_(vac_avail)', 'necs_from_param_(vac_eff)',  
    ...
    for country in countries:
        ...
        for task_name, param_idx, target in itertools.product(task_names, range(40), targets):
            ...
            if expr_name == 'necs_from_contact': direct_effects_task_name = f'direct_effects_from_contact_({expr_param})'
            else: direct_effects_task_name = f'direct_effects_from_param_({expr_param})'
            ...
            if expr_param != 'vac_dur': anal_data[sheet_name] = {'optimal_ind': [], 'optimal_dir': [], 'worst_ind': [], 'worst_dir': [], 'effect_corr': [], 'min_max_corr': [], 'optimal_indx': [], 'optimal_dirx': [], 'worst_indx': [], 'worst_dirx': [], 'effect_corrx': []}
            else: anal_data[sheet_name] = {'optimal': [], 'worst': [], 'alloc_corr': []}
            if target == 'c': 
                sheet_name_x = f'{task_name}_{param_idx}_{basic_params.country_abbr[country]}'
                anal_data[sheet_name_x] = {'alloc_corr': []}
            if expr_param != 'vac_avail': vac_avail = 0.3
            else: vac_avail = np.linspace(0.12, 0.9, 40)[param_idx]
            for file_idx in range(40):
                if expr_name in ['necs_from_param', 'necs_from_contact']: ifrs = calc_params['ifrs']
                else: ifrs = rgd.country_ifrs[country][expr_param.split('_')[1]][file_idx * 40][param_idx]
