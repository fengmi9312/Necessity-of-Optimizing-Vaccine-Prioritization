...
def analyze(expr_data):
    ...
    for task_name, country, target, acc_type in itertools.product(task_names, countries, targets, acc_types):
        ...
        for r0_idx, param_idx in itertools.product(range(r0_amount), range(param_amount)):
            ...
            anti_dist_data = expr_data['necs_from_time_course_fixd_(param)'][r0_idx * param_amount + 28]['dist']
            append_name = f'{{{expr_param}_{basic_params.country_abbr[country]}_{vac_dur}}}'
            ...
            anti_min_val = anti_dist_data[f"min_{anti_target}_{acc_type}_{append_name}"].to_numpy() @ pop_coef
