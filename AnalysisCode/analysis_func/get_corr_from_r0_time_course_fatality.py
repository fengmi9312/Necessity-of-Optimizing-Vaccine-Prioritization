...
def analyze(expr_data):
    ...
    anal_data = {}
    task_names = ['necs_from_time_course_fixd_fatality_(param)']
    r0_amount = 40
    param_amount = 40
    vac_dur_offset = 2
    ...
    for country in countries:
        ...
        for task_name, param_idx, target in itertools.product(task_names, range(param_amount), targets):
            ...
            for r0_idx in range(r0_amount):
                ...
                alloc_data_c = expr_data['necs_from_time_course_fixd_(param)'][r0_idx * param_amount + 28]['alloc']
                ...
                direct_effects_coef = expr_data[direct_effects_task_name][r0_idx][f'fixd_{basic_params.country_abbr[country]}_{r0_idx}'][str(28)]
                ...
                anal_data[sheet_name]['effect_corr'].append(anal_func.cosine_similarity(direct_effects_eff * coef_target[target], calc_params['populations'] * contact_eff))
                anal_data[sheet_name]['optimal_ind'].append(anal_func.cosine_similarity(alloc_optimal * calc_params['populations'], contact_eff * calc_params['populations']))
                anal_data[sheet_name]['optimal_dir'].append(anal_func.cosine_similarity(alloc_optimal * calc_params['populations'], direct_effects_eff * coef_target[target]))
                anal_data[sheet_name]['worst_ind'].append(anal_func.cosine_similarity(alloc_worst * calc_params['populations'], contact_eff * calc_params['populations']))
                anal_data[sheet_name]['worst_dir'].append(anal_func.cosine_similarity(alloc_worst * calc_params['populations'], direct_effects_eff * coef_target[target]))
                ...
                if target == 'c':
                    anal_data[sheet_name_x]['alloc_corr'].append(pearsonr(alloc_data_c[f'min_c_{append_name}'].to_numpy() * calc_params['populations'], alloc_data[f'min_d_{append_name}'].to_numpy() * calc_params['populations'])[0])
    return anal_data
