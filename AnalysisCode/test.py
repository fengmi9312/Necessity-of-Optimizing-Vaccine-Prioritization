...
additional_task_dict = {'necs_line_from_param': {'delay_8': 40, 'delay_10': 40, 'delay_0': 40, 'delay_13': 40, 'delay_26': 40, 'delay_39': 40}}
...
anal_types = ['necs_from_r0_factor', 'necs_line_from_r0', 'corr_from_r0_factor', 'corr_line_from_r0', 'alloc_from_r0_factor', 'necs_from_growth', 'fraction_from_time_with_example', 'alloc_from_age_with_example' 'dist_diff_from_r0']
anal_data = {}

additional_anal_types = ['corr_line_from_r0']
for anal_type in additional_anal_types:
...
fc.heatmap_necs_from_r0_facotr(anal_data, 'necs_from_param', 'delay', target = 'c')
if save_enabled: plt.savefig(os.path.join(subfig2_path, f'plot_necs_from_param_delay_c.{file_type}'))

plt.figure()
plt.plot(np.exp(np.arange(1600) * 0.075 / 40), anal_data['corr_line_from_r0']['necs_line_from_fatality_(coef_beta_1_39)_d_us']['optimal'], '.', c = 'tab:red')
plt.plot(np.exp(np.arange(1600) * 0.075 / 40), anal_data['corr_line_from_r0']['necs_line_from_fatality_(coef_beta_1_39)_d_us']['worst'], '.', c = 'tab:blue')
plt.xscale('log')


fc.heatmap_optimal_from_r0_facotr(anal_data, 'necs_from_param', 'delay', target = 'd')


fc.plot_necs_from_growth(anal_data, target = 'c')      
